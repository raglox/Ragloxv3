...
class TestMissionChat:
    ...
    def test_send_chat_message_success(self, client: httpx.Client, created_mission: Dict[str, Any], 
        ...
        response = client.post(f"/api/v1/missions/{mission_id}/chat", json=sample_chat_request)
    ...
    def test_send_chat_message_with_related_ids(self, client: httpx.Client, created_mission: Dict[str, Any]) -> None:
        ...
        response = client.post(f"/api/v1/missions/{mission_id}/chat", json=chat_request)
        assert response.status_code == 200
        data = response.json()
    ...
    def test_send_chat_message_validation_error(self, client: httpx.Client, created_mission: Dict[str, Any]) -> None:
        """Test that sending a chat message with invalid data returns 422."""
        mission_id = created_mission["mission_id"]
        
        # Missing required content field
        response = client.post(f"/api/v1/missions/{mission_id}/chat", json={})
        assert response.status_code == 422
        
        # Empty content
        response = client.post(f"/api/v1/missions/{mission_id}/chat", json={"content": ""})
        assert response.status_code == 422
        
        # Content too long (exceeds 4096 chars)
        long_content = "a" * 4097
        response = client.post(f"/api/v1/missions/{mission_id}/chat", json={"content": long_content})
        assert response.status_code == 422

    def test_send_chat_message_mission_not_found(self, client: httpx.Client, mission_id: str, 
                                                sample_chat_request: Dict[str, Any]) -> None:
        """Test that sending a chat message to non-existent mission returns 404."""
        response = client.post(f"/api/v1/missions/{mission_id}/chat", json=sample_chat_request)
        assert response.status_code == 404

    def test_get_chat_history_success(self, client: httpx.Client, created_mission: Dict[str, Any]) -> None:
        """Test that GET /api/v1/missions/{mission_id}/chat returns 200 OK."""
        mission_id = created_mission["mission_id"]
        response = client.get(f"/api/v1/missions/{mission_id}/chat")
        assert response.status_code == 200
        data = response.json()
    ...
    def test_get_chat_history_with_limit(self, client: httpx.Client, created_mission: Dict[str, Any]) -> None:
        """Test getting chat history with a limit parameter."""
        mission_id = created_mission["mission_id"]
        response = client.get(f"/api/v1/missions/{mission_id}/chat?limit=5")
        assert response.status_code == 200
        data = response.json()
        assert isinstance(data, list)
    ...
    def test_get_chat_history_validation_error(self, client: httpx.Client, created_mission: Dict[str, Any]) -> None:
        """Test that getting chat history with invalid limit returns 422."""
        mission_id = created_mission["mission_id"]
        
        # Negative limit
        response = client.get(f"/api/v1/missions/{mission_id}/chat?limit=-1")
        assert response.status_code == 422
        
        # Zero limit (not allowed)
        response = client.get(f"/api/v1/missions/{mission_id}/chat?limit=0")
        assert response.status_code == 422
        
        # Non-integer limit
        response = client.get(f"/api/v1/missions/{mission_id}/chat?limit=abc")
        assert response.status_code == 422

    def test_get_chat_history_mission_not_found(self, client: httpx.Client, mission_id: str) -> None:
...
class TestChatWorkflow:
    ...
    def test_chat_interaction_workflow(self, client: httpx.Client, running_mission: Dict[str, Any]) -> None:
        ...
        response = client.post(f"/api/v1/missions/{mission_id}/chat", json=second_request)
        assert response.status_code == 200
